...
<article {{ $attributes->merge([
    'class' => 'group rounded-2xl border border-[#2d3044] bg-[#1b1d2a]/[0.88] px-4 py-4
    cursor-pointer
    ...
    <div class="flex items-start gap-3 sm:items-center sm:gap-4"
        ...
        <button type="button" wire:click.stop="toggleComplete({{ $task->id }})" wire:loading.class="animate-pulse"
            ...
            class="mt-0.5 grid h-5.5 w-5.5 flex-none place-items-center rounded-full border-[1.5px] border-[#596079] text-surface-sidebar transition-colors duration-180 motion-reduce:transition-none hover:scale-[1.06] hover:border-(--check-icon-color) hover:bg-[var(--check-icon-color)/0.12] data-[completed=true]:border-(--check-icon-color) data-[completed=true]:bg-(--check-icon-color) cursor-pointer"
